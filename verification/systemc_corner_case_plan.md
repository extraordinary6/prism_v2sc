...
- 2026-07-10：新增带 `interface` 的 ICB-to-APB 真实设计 gate：`verification/cases/consistency/icb_apb_bridge_consistency.py` 在不修改外部 RTL 的前提下构造临时可综合快照和平坦 wrapper，保留 DES，忽略 `CHECK`/bind 验证内容。转换 8 个 source、访问 6 个 module、0 error diagnostic，generated top C++14/SystemC 编译通过；RTL(VCS) vs SystemC 的 36 个 ICB/APB 事务事件一致，覆盖 mask/error/register、CDC FIFO、四路 APB 读写和不同 wait-state。该轮修复了 interface instance/member/constructor flatten、函数局部变量、直接 StatementList、多维 assignment pattern、ascending packed range、参数化跨层宽度和参数化大位宽 SystemC 类型选择；详见 `verification/notes/icb_apb_bridge_real_design_eval.md`。
- 2026-07-10：OFDM FFT/IFFT 9 个 consistency case、MHSA 三个 keypoint case 和全量 VCS/SystemC equivalence 回归全部通过。新增 assertion-only 回归，明确 concurrent/immediate assertion 在可综合视图中降为 noop，且 assertion-only procedural block 不生成空 `SC_METHOD`。
- 2026-07-09：新增 MHSA ICB 真实设计验证入口：`verification/cases/conversion/mhsa_icb_smoke.py` 完成外部 `/home/MicroE/MHSA` `icb_mhsa` 转换和 generated top header C++14/SystemC 编译 smoke；`verification/cases/consistency/mhsa_keypoint_consistency.py` 完成 RTL(VCS) vs generated SystemC 近似周期精确关键节点检查，覆盖 `icb_mhsa` 8 个 CSR/ICB-USRAM 样本、`pe` 9 个 signed datapath 样本、`scale_core` 7 个 byte scaling 样本。该验证暴露并修复了多维 unpacked array、array-element port bridge、concat-LHS assignment、expression port bridge、indexed part-select、loop-local procedural `for` 和 level-sensitive `always_ff @(clk)` sensitivity 等真实设计问题；详见 `verification/notes/mhsa_icb_real_design_eval.md`。
