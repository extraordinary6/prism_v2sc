...
| **Literals** | sized (`8'hFF`, `3'b010`, `4'd5`, 65-bit based literals), signed based (`8'shFF`), unsized decimal; integer `value` field reflects the actual bit pattern and signed based literals also carry `signed_value`; based literals wider than 64 bits emit string-constructed `sc_biguint` / `sc_bigint` values; implicit real-to-integral constant conversions preserve slang's converted integer value for LUT-style assignments (`ofdm_fft_smoke`) |
...
| **Multi-writer aggregation** | multiple procedural blocks or continuous assignments writing different constant-foldable bit/part-select slices of the same parent signal land in one shadow-driven assembler; slice assignment width conversion is preserved — procedural behavior is trace-verified by `slice_writers`, arithmetic procedural/continuous slices are covered by unit regressions and tinyNPU |
