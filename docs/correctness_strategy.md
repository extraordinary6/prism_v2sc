...
| Unit tests | IR lowering shape, codegen text, per-module file layout, expression coverage, diagnostics, model providers, staged projects, power analysis, instrumentation, and reporting. | `tests/`, run with `python -m pytest -q` (218 tests at the current baseline). |
